...
export function analysis_to_tokens(analysis: CssAnalysis): Tokens {
  return {
        ...
        for (let color of group_colors) {
          ...
          if (color_token !== null) {
            ...
            let name = `${color_dict.get(group)}-${hash(
              [colorSpace, ...components, alpha].join("")
            )}`;

            let items_per_context = analysis.values.colors
              .itemsPerContext as ItemsPerContext;
            let new_properties = Object.entries(items_per_context).reduce(
              (acc, [property, collection]) => {
                ...
                if (
                  collection.uniqueWithLocations &&
                  color in collection.uniqueWithLocations
                ) {
                  return acc.add(property);
                }
                return acc;
            ...
            if (colors[name]) {
              let old_properties =
                colors[name].$extensions[EXTENSION_CSS_PROPERTIES];
    ...
    font_size: (() => {
      let font_sizes = Object.create(null) as Record<
        TokenID,
        UnparsedToken | DimensionToken
      >;
      ...
      for (let font_size in unique) {
        ...
        if (parsed === null) {
          ...
          if (font_sizes[name]) {
            font_sizes[name].$extensions[EXTENSION_USAGE_COUNT] +=
      ...
      for (let line_height in unique) {
        ...
        if (parsed === null) {
          ...
          if (parsed.unit === "px" || parsed.unit === "rem") {
            let name = `lineHeight-${hash(
              parsed.value.toString() + parsed.unit
            )}`;
            if (line_heights[name]) {
              line_heights[name].$extensions[EXTENSION_USAGE_COUNT] +=
                extensions[EXTENSION_USAGE_COUNT];
    ...
    box_shadow: (() => {
      let shadows = Object.create(null) as Record<
        TokenID,
        ShadowToken | UnparsedToken
      >;
    ...
    duration: (() => {
      let durations = Object.create(null) as Record<
        TokenID,
        DurationToken | UnparsedToken
      >;
      ...
      for (let duration in unique) {
        ...
        if (is_valid) {
          ...
          if (durations[name]) {
            durations[name].$extensions[EXTENSION_USAGE_COUNT] +=
              extensions[EXTENSION_USAGE_COUNT];
          } else {
            durations[name] = {
              $type: "duration",
    ...
    easing: (() => {
      let easings = Object.create(null) as Record<
        TokenID,
        UnparsedToken | CubicBezierToken
      >;
